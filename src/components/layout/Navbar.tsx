    ...
    <header className="sticky top-0 z-30 bg-surface/85 backdrop-blur-md border-b border-border px-6 h-16 flex items-center justify-between transition-colors">
      ...
      <form onSubmit={handleSearchSubmit} className="relative w-72 md:w-96">
        ...
        <input
          ...
          className="w-full pl-9 pr-4 py-1.5 bg-surface-secondary border border-border rounded-lg text-xs font-mono text-foreground placeholder:text-foreground-muted focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all"
        />
      </form>
      ...
      <div className="flex items-center space-x-3">
        ...
        <div className="hidden lg:flex items-center space-x-2 px-2.5 py-1 rounded-md bg-danger-surface border border-danger-border text-danger text-xs font-mono">
          <ShieldAlert className="w-3.5 h-3.5" />
          <span>THREAT: <strong className="font-semibold">DEFCON 2</strong></span>
        </div>

        {/* AI Engine Status */}
        <div className="hidden sm:flex items-center space-x-1.5 px-2.5 py-1 rounded-md bg-surface-secondary border border-border text-xs font-mono text-foreground-muted">
        ...
        <div className="relative" ref={themeMenuRef}>
          <button
            ...
            className="flex items-center space-x-1.5 px-2.5 py-1.5 rounded-lg bg-surface border border-border hover:bg-surface-secondary text-foreground text-xs font-medium transition-colors btn-press cursor-pointer"
          ...
          {showThemeMenu && (
            <div className="absolute right-0 mt-2 w-56 bg-surface border border-border rounded-xl shadow-lg p-1.5 z-50 animate-in fade-in slide-in-from-top-1 duration-150">
              <div className="px-2.5 py-1.5 text-[11px] font-mono font-semibold uppercase text-foreground-muted border-b border-border-subtle mb-1">
                Select Theme
              </div>
        ...
        <div className="relative" ref={notifRef}>
          <button
            ...
            className="relative p-2 rounded-lg bg-surface hover:bg-surface-secondary border border-border text-foreground-muted hover:text-foreground transition-colors btn-press cursor-pointer"
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-80 md:w-96 bg-surface border border-border rounded-xl shadow-lg p-3 z-50 animate-in fade-in slide-in-from-top-1 duration-150">
              ...
              <div className="space-y-1.5">
                {mockAlerts.map((alt) => (
                  <div
                    ...
                    className="p-2.5 rounded-lg bg-surface-secondary/70 border border-border hover:border-primary cursor-pointer transition-colors"
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[9px] font-mono font-bold px-1.5 py-0.2 rounded bg-danger-surface text-danger border border-danger-border">
                        {alt.level}
                      </span>
